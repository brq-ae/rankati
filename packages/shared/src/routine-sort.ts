import type { PeriodUnit, Routine } from './index';

/**
 * The Routines climb order (ADR 0066 + its v0.18 pace extension) — pure logic, SHARED by every client
 * (ADR 0088). The caller passes the local day `on`, so it stays clock-free and unit-testable. It lives in
 * `@rankati/shared` so the web (which bundles this source) and the api/bot (which run the built `dist`)
 * compute the IDENTICAL routines order from one function (like `computePin`/`computeLogStats`).
 *
